...
int gen_insertion_sort(void* ar, int n, size_t size, int compare(const void*, const void*)) {
  ...
  for(int i = 1; i < n; i++){
    int j = i;
    while(compare((void*)(ar + size*j), (void*)(ar + size*(j-1))) < 0 && j > 0){
      swap((void*)(ar + size*j), (void*)(ar + size*(j-1)), size);
      counter++;
      j--;
    }  
  }
  return counter;
    ...
    // for(int j = i; j > 0; j--){
    //   if(compare((unsigned char *)(ar + size*j), (unsigned char *)(ar + size*(j-1))) > 0){
    //     swap((unsigned char *)(&ar + size*j), (unsigned char *)(&ar + size*(j-1)), size);
    //     counter++;
    //   }  
    // }
